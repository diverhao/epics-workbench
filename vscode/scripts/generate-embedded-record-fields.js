...
if (!dbdRoot) {
  console.error(
    "Usage: node scripts/generate-embedded-record-fields.js /path/to/epics-base/dbd",
  );
  process.exit(1);
}
...
function extractFieldsForRecordTypeFromDbdFile(filePath, recordType, visitedPaths) {
  ...
  while ((match = recordTypeRegex.exec(text))) {
    ...
    addStandaloneDbdFieldsAndIncludes(
      labels,
      block.body,
      normalizedPath,
      visitedPaths,
...
function addStandaloneDbdFieldsAndIncludes(labels, text, sourcePath, visitedPaths) {
  const fieldRegex = /field\(\s*([A-Z0-9_]+)\s*,/g;
  let fieldMatch;
  while ((fieldMatch = fieldRegex.exec(text))) {
    labels.add(fieldMatch[1]);
  }
