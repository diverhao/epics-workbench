...
if (!dbdRoot) {
  console.error(
    "Usage: node scripts/generate-embedded-record-field-initials.js /path/to/epics-base/dbd",
  );
  process.exit(1);
}
...
for (const [fileName, filePath] of [...dbdFilesByName.entries()].sort()) {
  ...
  const fieldInitials = extractFieldInitialsForRecordTypeFromDbdFile(
    filePath,
    recordType,
    new Set(),
  );
  const sortedEntries = [...fieldInitials.entries()].sort(([left], [right]) =>
...
function extractFieldInitialsForRecordTypeFromDbdFile(filePath, recordType, visitedPaths) {
  ...
  while ((match = recordTypeRegex.exec(text))) {
    ...
    addStandaloneDbdFieldInitialsAndIncludes(
      fieldInitials,
      block.body,
      normalizedPath,
      visitedPaths,
  ...
  return fieldInitials;
}

function addStandaloneDbdFieldInitialsAndIncludes(
  fieldInitials,
  text,
  sourcePath,
  visitedPaths,
  ...
  while ((fieldMatch = fieldRegex.exec(text))) {
    ...
    const initialMatch = block.body.match(/initial\(\s*"((?:[^"\\]|\\.)*)"\s*\)/);
    if (initialMatch && !fieldInitials.has(fieldName)) {
      fieldInitials.set(fieldName, initialMatch[1]);
    }
